...
class GroqScheduleService {
  ...
  static String get _systemPrompt {
    ...
    final tomorrow = now.add(Duration(days: 1)).toIso8601String().split('T')[0];
    final dayAfterTomorrow = now.add(Duration(days: 2)).toIso8601String().split('T')[0];
...
ATURAN RESPONSE:
1. Jika user meminta jadwal belajar, WAJIB response HANYA dalam format JSON yang valid
2. JANGAN tambahkan teks apapun sebelum atau sesudah JSON
...
4. PENTING: Untuk date gunakan format YYYY-MM-DD mulai dari HARI INI ($today)
5. Contoh tanggal yang benar:
   - Hari ini: $today
   - Besok: $tomorrow
   - Lusa: $dayAfterTomorrow
6. Untuk time dan endTime gunakan format HH:MM (24 jam)
7. Priority: "low", "medium", "high", "urgent"
8. Category: "study", "work", "personal", "other"
9. Default durasi 3 hari jika tidak disebutkan
10. Waktu belajar realistis: 1-3 jam per sesi
11. Berikan jeda istirahat yang cukup
12. Sesuaikan dengan roadmap/course yang ada jika disebutkan
...
- Waktu sekarang: ${now.hour.toString().padLeft(2, '0')}:${now.minute.toString().padLeft(2, '0')}
- Hari: ${_getDayName(now.weekday)}

GUNAKAN INFORMASI INI untuk membuat jadwal yang realistis dan sesuai waktu!
''';
  }
  ...
  static Future<String> sendMessage(List<ChatMessage> messages) async {
    ...
    try {
      ...
      print('📅 Current date context: ${DateTime.now().toIso8601String().split('T')[0]}');
  ...
  static bool isScheduleResponse(String response) {
    try {
      print('🔍 Checking if response is schedule...');
      final trimmed = response.trim();
      print('📄 Response preview: ${trimmed.substring(0, trimmed.length > 200 ? 200 : trimmed.length)}...');
      ...
      final jsonEnd = trimmed.lastIndexOf('}') + 1;
      ...
      final jsonString = trimmed.substring(jsonStart, jsonEnd);
      print('📋 Extracted JSON: ${jsonString.substring(0, jsonString.length > 200 ? 200 : jsonString.length)}...');
      
      final json = jsonDecode(jsonString);
      final isSchedule = json is Map<String, dynamic> && 
             (json['type'] == 'schedule' || json.containsKey('tasks'));
  ...
  static List<TaskModel> parseScheduleResponse(String response) {
    try {
      ...
      return tasks.map<TaskModel>((taskJson) {
        ...
        print('📅 Parsing task date: $dateStr → ${date.toIso8601String().split('T')[0]}');
        
        // Ensure date is not in the past
        final today = DateTime(now.year, now.month, now.day);
        if (date.isBefore(today)) {
          print('⚠️ Date $dateStr is in the past, adjusting to today');
          date = today;
        }
